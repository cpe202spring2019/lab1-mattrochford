...
def bin_search(target, low, high, int_list):  # must use recursion
    ...
    if (high-low)//2 == 0: # Check if list only has one number left
    	if target != middle: # If last number doesnt match return empty list
    		return []
    ...
    if target == int_list[middle]: # If target matches middle number
   	...
   	# elif low < 0 or not float(low).is_integer(): # Check first search index
    #     raise ValueError('Low value cannot be less than zero and must be integer')

   	# elif high > len(int_list)-1 or not float(high).is_integer(): # Check first search index
    #     raise ValueError('Low value cannot be less than zero and must be integer')
